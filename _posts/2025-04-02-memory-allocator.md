...
梳理 ptmalloc、tcmalloc 和 jemalloc 内存分配器，并进行内存相关实验，工具：[Massif](https://valgrind.org/docs/manual/ms-manual.html)、[AddressSanitizer](https://github.com/google/sanitizers/wiki/AddressSanitizer)、[Memory Leak火焰图](https://www.brendangregg.com/FlameGraphs/memoryflamegraphs.html)。
...
并利用 [Massif](https://valgrind.org/docs/manual/ms-manual.html)、[AddressSanitizer](https://github.com/google/sanitizers/wiki/AddressSanitizer) 进行内存相关实验，以及 [并发与异步编程（三） -- 性能分析工具：gperftools和火焰图](https://xiaodongq.github.io/2025/03/14/async-io-example-profile/) 中未展开的 [Memory Leak and Growth火焰图](https://www.brendangregg.com/FlameGraphs/memoryflamegraphs.html)。
...
不过看RocksDB里使用时，几个Sanitizer和jemalloc可能存在一些冲突问题：
