...
* 1）服务端重传报文的过程中，客户端刚好把网线插回去了，拔掉网线并不会改变客户端的 TCP 连接状态，并且还是处于 ESTABLISHED 状态，所以这时客户端是可以正常接收服务端发来的数据报文的，然后客户端就会回 ACK 响应报文。

* 2）重传报文的过程中，客户端一直没有将网线插回去
...
TCP keepalive 机制 （TCP 保活机制），需要手动开启，比如Redis里，设置`SO_KEEPALIVE`：
...
tcp keepalive相关参数：（即TCP`内核态`保活机制 `7200+75*9 = 2小时11分15秒` 才中断连接。此外上层`应用`可以另行实现探测机制进行保活判断）
...
* 跟`拔掉网线`场景是一样的，无法被对端感知。对于有无数据传输、是否开启TCP keepalive保活机制，表现一样。
* 所以如果在没有数据传输，并且没有开启 TCP keepalive 机制时，对端的 TCP 连接将会一直处于 `ESTABLISHED` 连接状态。

**2、进程崩溃：**

* 即使没有数据传输、没开启TCP keepalive机制，进程崩溃时**内核可以感知**到。
    * 内核会向对端发送`FIN`报文，后续的挥手过程也都是在内核完成，并不需要进程的参与，会**正常进行四次挥手**。
* 对于进程崩溃后立即重启，收到对端之前TCP连接的报文时，都会回复`RST`报文，以断开连接

具体分析请参考：[4.12 TCP 连接，一端断电和进程崩溃有什么区别？](https://www.xiaolincoding.com/network/3_tcp/tcp_down_and_crash.html)

## 4. 如何关闭一个TCP连接

`tcpkill`和`killcx`两个工具都是通过**伪造`RST`报文**来关闭指定的 TCP 连接，但是它们拿到正确的序列号的实现方式是不同的。

* tcpkill 工具是在双方进行 TCP 通信时，拿到对方下一次期望收到的序列号，然后将序列号填充到伪造的 RST 报文，并将其发送给对方，达到关闭 TCP 连接的效果。
    * tcpkill 工具属于被动获取，双方通信时才能获取正确序列号，这种方式**无法关闭非活跃的 TCP 连接**
* killcx 工具是主动发送一个 SYN 报文，对方收到后会回复一个携带了正确序列号和确认号的 ACK 报文，这个 ACK 即 `Challenge ACK`，这时就可以拿到对方下一次期望收到的序列号，然后将序列号填充到伪造的 RST 报文，并将其发送给对方，达到关闭 TCP 连接的效果。
    * killcx 工具则是属于主动获取，**无论 TCP 连接是否活跃，都可以关闭**
    * 利用 SEQ/ACK 号伪造两个`RST`报文分别发给客户端和服务端，从而关闭双方的连接

killcx通过`Challenge ACK`关闭连接原理流程示意图：

![killcx通过Challenge ACK关闭连接原理](/images/killcx_process.png)  
[出处](https://www.xiaolincoding.com/network/3_tcp/challenge_ack.html)

### 4.1. tcpkill使用

`tcpkill`来自`dsniff`工具集。原理跟`tcpdump`差不多，会通过`libpcap`库抓取符合条件的包。

* 官网（个人项目）：[dsniff](https://www.monkey.org/~dugsong/dsniff/)
...
1) `python -m http.server`启动服务端，默认8000
...
梳理异常情况下的TCP连接状态；并介绍`tcpkill`和`killcx`工具，如何关闭一个TCP连接，以及其中的基本原理。

TODO：

* 后续阅读工具的代码，考虑用Cpp/Go/Rust实现小工具
* CentOS停止维护，很多包不好找，考虑替换开发环境，RockyLinux/AlmaLinux/Ubuntu
